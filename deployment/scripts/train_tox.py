...
def _fit_arm(X_tr, y_tr, w_tr, X_va, y_va, arm, args):
    """Fit one arm of the two-stage champion on a shared feature matrix.

    arm='regression' -> reg:squarederror on viability; arm='binary' -> binary:logistic on the
    toxic indicator (viability < TOX_THRESHOLD). Both use the GKDE Sample_weights (w_tr) and early-
    stop on valid toxic-detection PR-AUC. Returns (booster, best_iteration, valid_prediction)."""
    if arm == "binary":
    ...
    dvalid = xgb.DMatrix(X_va, label=y_va_fit)
    ...
    if arm == "binary":
        ...
        params["base_score"] = float(np.average(y_tr, weights=w_tr))
    booster = xgb.train(
        params, dtrain, num_boost_round=args.num_boost_round,
        evals=[(dvalid, "valid")], custom_metric=make_pr_auc_metric(arm),
        maximize=True, early_stopping_rounds=args.early_stopping, verbose_eval=args.verbose_eval,
    )
    best = int(getattr(booster, "best_iteration", booster.num_boosted_rounds() - 1))
    va_pred = booster.predict(dvalid, iteration_range=(0, best + 1))
    return booster, best, va_pred
...
def train_fold(split_dir, save_dir, cv_fold, target_col, mode, args):
    ...
    # Champion: TOX uses 16 PCA dims (regularizes the OOD toxic signal), not the delivery 64.
    n_molgpka_pca = MOLGPKA_N_PCA_TOX
    molgpka_pca = PCA(n_components=n_molgpka_pca, random_state=0).fit(train_molgpka_block)
    logger.info(f"MolGpKa PCA({n_molgpka_pca}) explained_variance="
    ...
    # ---- CHAMPION: two-stage detect-then-regress (default) ----
    if args.two_stage:
        logger.info("Two-stage champion: training regressor + classifier arms on shared features ...")
        reg_b, reg_it, reg_va = _fit_arm(X_tr, y_tr, w_tr, X_va, y_va, "regression", args)
        clf_b, clf_it, clf_va = _fit_arm(X_tr, y_tr, w_tr, X_va, y_va, "binary", args)
        tox_va = two_stage_score(reg_va, clf_va)  # higher = more toxic
        is_tox = (y_va < TOX_THRESHOLD).astype(int)
        have_both = 0 < is_tox.sum() < len(is_tox)
        metrics = {
            "roc_auc": float(roc_auc_score(is_tox, tox_va)) if have_both else float("nan"),
            "pr_auc": float(average_precision_score(is_tox, tox_va)) if have_both else float("nan"),
        }
        logger.info(f"Fold {cv_fold} two-stage reg_best={reg_it} clf_best={clf_it}  "
                    f"[valid] roc_auc={metrics['roc_auc']:.4f} pr_auc={metrics['pr_auc']:.4f}")
        final_dir = os.path.join(save_dir, "final_model")
        os.makedirs(final_dir, exist_ok=True)
        reg_b.save_model(os.path.join(final_dir, REG_MODEL_FILE))
        clf_b.save_model(os.path.join(final_dir, CLF_MODEL_FILE))
        with open(os.path.join(final_dir, "model_meta.pkl"), "wb") as fh:
            pickle.dump({
                "model_version": MODEL_VERSION,
                "task": TWO_STAGE_TASK,
                "mode": mode, "target_col": target_col, "tox_threshold": TOX_THRESHOLD,
                "extra_dim": len(extra_cols), "extra_cols": extra_cols,
                "emb_dim": int(X_tr.shape[1] - len(extra_cols)), "emb_pooling": "masked_mean",
                "chemotype_features": bool(args.chemotype_features),
                "rdkit_features": bool(args.rdkit_features),
                "molgpka_pooling": MOLGPKA_POOLING, "molgpka_n_pca": n_molgpka_pca,
                "reg_best_iteration": reg_it, "clf_best_iteration": clf_it,
                "reg_model_file": REG_MODEL_FILE, "clf_model_file": CLF_MODEL_FILE,
                "tox_score": "rank_avg(-reg, clf)", "selection_metric": "valid_pr_auc",
                **{f"valid_{k}": float(v) for k, v in metrics.items()},
            }, fh)
        logger.info(f"Fold {cv_fold} two-stage done -> {final_dir}")
        return metrics, reg_it
...
def parse_args(argv=None):
    ...
    parser.add_argument("--regression", action="store_true",
                        help="Legacy single-arm regression on viability (pre-champion). The DEFAULT "
                             "is the two-stage champion (reg + clf rank-averaged, MolGpKa PCA-16).")
...
def main():
    ...
    if sum([args.multiclass, args.classifier, args.focal, args.regression]) > 1:
        sys.exit("ERROR: pass at most one of --classifier / --multiclass / --focal / --regression.")
    # Two-stage champion is the DEFAULT; any explicit single-arm flag opts out.
    args.two_stage = not (args.multiclass or args.classifier or args.focal or args.regression)

    arm = ("two-stage detect-then-regress (reg + clf, rank-averaged) [CHAMPION]" if args.two_stage
           else "multi:softprob 3-class {<0.8,0.8-0.9,>=0.9}" if args.multiclass
    ...
    target_desc = ("viability + P(toxic)" if args.two_stage else "ordinal class" if args.multiclass
    ...
    print(f"Features        : ChemBERTa-77M-MTR (masked-mean, raw 384d) + handcrafted + "
          f"MolGpKa ({MOLGPKA_POOLING}-pool, PCA-{MOLGPKA_N_PCA_TOX})"
