...
def train_fold(split_dir, save_dir, cv_fold, target_col, mode, args):
    ...
    # --- Feature ablation (A/B): drop named handcrafted X_val columns BEFORE anything downstream,
    #     so they never enter the MolGpKa block / scaler / extra_cols (the model is fit without them).
    if getattr(args, "drop_features", None):
        drop = [c for c in args.drop_features if c in df_tr_extra.columns]
        missing = [c for c in args.drop_features if c not in df_tr_extra.columns]
        df_tr_extra = df_tr_extra.drop(columns=drop)
        df_va_extra = df_va_extra.drop(columns=drop)
        logger.info(f"ABLATION: dropped {len(drop)} handcrafted feature(s): {drop}"
                    + (f"  |  requested-but-absent: {missing}" if missing else ""))
...
def parse_args(argv=None):
    ...
    parser.add_argument("--drop_features", type=str, nargs="+", default=[],
                        help="Handcrafted X_val column names to ABLATE (drop before fitting), for "
                             "feature-ablation A/B tests (e.g. size descriptors).")
    if argv is None:
        argv = sys.argv[1:]
