"""
analyze_pw.py - Evaluate the DUET-LNP within-experiment pairwise XGBoost model.

Recomputes frozen ChemBERTa embeddings for the requested subset, concatenates the
scaled formulation features, predicts a scalar score per lipid with the saved XGBoost
booster, and reports ONLY within-experiment metrics:
...
Across-experiment / global metrics are intentionally omitted: the training objective
is invariant to a per-experiment additive constant in the score, so only
within-experiment comparisons are meaningful (see within_exp_pairwise_mse.py).
...
def load_model(model_dir, expected_target_col):
    ...
    with open(os.path.join(final_dir, "model_meta.pkl"), "rb") as fh:
        meta = pickle.load(fh)
    if meta.get("model_version") != MODEL_VERSION_PW_XGB:
        raise ValueError(
            f"Unsupported pairwise artifact in {model_dir} "
            f"(model_version={meta.get('model_version')!r}). Retrain with the current train_pw.py."
        )
...
def evaluate_fold(split_folder, model_folder, cv, tvt, results_base, target_col, device, tokenizer, encoder):
    model_dir = f"../new_data/crossval_splits/{model_folder}/fold_{cv}/model_pw_{cv}"
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Evaluate DUET-LNP within-experiment pairwise XGBoost model.")
