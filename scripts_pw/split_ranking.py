"""
split_ranking.py - Full data splitting for DUET-LNP single-target ranking.

Usage:
    python split_ranking.py <split_spec> <mode> [options]

The split unit is always Experiment_ID.  No experiment may appear in more than
one of train, valid, or test for a fold.  Split specs may contain
Train_or_split values of train, split/split_context, or test.

Produces true K-fold CV: all splittable pool experiments are assigned to K
groups. For outer fold i, group i = test, group (i+1)%K = valid, the
remaining K-2 groups + permanent-train = train. Each fold writes its own
test/, train/, valid/ under fold_{i}/.
"""
...
def build_splits(spec_path, mode, data_dir, output_name, cv_folds, seed,
    ...
    all_df = pd.read_csv(os.path.join(data_dir, data_fname))
    ...
    test_hold_frac = test_frac / (1.0 - eho_frac)
...
def parse_args(argv=None):
    parser = argparse.ArgumentParser(description="Data splitting for DUET-LNP single-target ranking.")
    parser.add_argument("split_spec", help="Split-spec CSV name under ../new_data/crossval_split_specs/")
    parser.add_argument("mode", choices=["del", "tox"])
...
def main():
    ...
    output_name = args.output_name or f"{spec_stem}_rank_{args.mode}_B"
    ...
    build_splits(spec_path, args.mode, args.data_dir, output_name, args.cv, args.seed)
