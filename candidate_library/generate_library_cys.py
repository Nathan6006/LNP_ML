...
def build_for_head(head_row, starters, subtrees, tail_db_counts):
    ...
    for (l_ab, t_ab), sub_smi in subtrees.items():
        for s_ab, s_smi in starters:
            ...
            n_tails = 4 if l_ab.endswith("K") else 2
...
def main() -> int:
    ...
    ap.add_argument("--dedup-against", type=Path, default=None,
                    help="CSV with a 'smiles' column; drop any molecule already present "
                         "there (canonical-SMILES identity). Used to keep only lipids NOT "
                         "in a previously built library.")
    args = ap.parse_args()
    ...
    # ---- dedup against an existing library (canonical-SMILES identity) ----------
    # Keep only lipids that are NOT already in a previously built library. Fragment
    # renaming / port-swaps mean identical molecules can carry different lipid_ids, so
    # the only correct identity test is the canonical neutral SMILES, not lipid_id.
    n_after_assembly = len(df)
    n_removed_dupes = 0
    if args.dedup_against is not None:
        ref = pd.read_csv(args.dedup_against, usecols=["smiles"])
        # Both sides are already RDKit-canonical: df.smiles was just emitted by MolToSmiles
        # above, and the reference library's smiles column was written the same way by this
        # same pipeline. So a direct string comparison is exact -- re-canonicalizing the
        # 360k-row reference is pure redundant cost (verified: 0/20000 change under re-canon).
        ref_set = set(ref["smiles"].astype(str))
        mask_dup = df["smiles"].isin(ref_set)
        n_removed_dupes = int(mask_dup.sum())
        df = df[~mask_dup].reset_index(drop=True)
        print(f"\ndedup: removed {n_removed_dupes:,} molecules already present in "
              f"{args.dedup_against}  ({n_after_assembly:,} -> {len(df):,})")

    lib_path = args.out / "eco_library.parquet"
    df.to_parquet(lib_path, index=False)

    # 8-column CSV matching the canonical lipid_library.csv schema
    csv_cols = ["lipid_id", "starter", "head", "linker", "tail",
                "smiles", "formula", "n_tails"]
    csv_path = args.out / "lipid_library.csv"
    ...
    manifest = {
        ...
        "n_excluded": (expected - n_after_assembly) if args.exclude_dead else 0,
        "dedup_against": str(args.dedup_against) if args.dedup_against else None,
        "n_removed_as_duplicate": n_removed_dupes,
        "charge_convention": (
    ...
    print(f"wrote {csv_path}  ({len(df):,} molecules, 8 cols)")
