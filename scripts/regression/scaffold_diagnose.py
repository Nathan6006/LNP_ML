...
"""
script that generates graphics and metrics that describe the distribution of the data based 
...
def plot_target_distribution_by_scaffold(df, top_n=15):
    """
    Creates a boxplot for the top N scaffolds.
    - X-axis: Simple labels (S1, S2, S3...) to keep the plot clean.
    - Y-axis: Target Value.
    - Outputs a separate CSV key to map S1 -> SMILES.
    """
    # 1. Select Top Scaffolds by Size
    ...
    # 2. Create a Mapping (S1, S2, S3...)
    # We map based on size (S1 = Largest Scaffold)
    ...
    # 3. Save the Legend/Key to CSV
    key_data = []
    for scaf, label in scaffold_to_label.items():
        count = scaffold_counts[scaf]
        # Calculate median toxicity for the report
        median_val = subset[subset['scaffold'] == scaf][TARGET_COL].median()
        key_data.append({
            'Label': label,
            'Count': count,
            'Median_Target': f"{median_val:.3f}",
            'Scaffold_SMILES': scaf
        })
    
    key_df = pd.DataFrame(key_data)
    key_path = os.path.join(OUTPUT_DIR, 'scaffold_legend.csv')
    key_df.to_csv(key_path, index=False)
    print(f"Saved scaffold legend to: {key_path}")

    # 4. Plot
    # Order the plot by Label (S1, S2, S3...)
    order = [f"S{i+1}" for i in range(len(top_scaffolds))]
    
    plt.figure(figsize=(12, 6))
    sns.boxplot(
        data=subset,
        x='Scaffold_Label',  # Clean S1, S2 labels
        y=TARGET_COL, 
        order=order,
        palette="viridis"
    )
    ...
    plt.close()
    print(f"Saved clean plot to {OUTPUT_DIR}/scaffold_target_distribution.png")
