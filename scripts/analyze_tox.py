...
def load_model(model_dir, expected_target_col):
    ...
    if meta.get("task") == TWO_STAGE_TASK:  # champion: two boosters
    ...
    return models, meta, scaler, extra_cols, molgpka_pca
...
def predict_fold(model_dir, split_dir, prefix, target_col, tokenizer, encoder, device):
    ...
    models, meta, scaler, extra_cols, molgpka_pca = load_model(model_dir, target_col)
    ...
    if task == TWO_STAGE_TASK:
        # Champion: rank-average the reg (-viability) and clf (P(toxic)) arms -> tox_score (higher =
        # more toxic). Downstream detection code treats it like a classifier score (higher=toxic).
        reg_pred = models["reg"].predict(xgb.DMatrix(X))
        clf_pred = models["clf"].predict(xgb.DMatrix(X))
        tox = two_stage_score(reg_pred, clf_pred)
        df = pd.DataFrame({"Experiment_ID": exp, "y_true": y_true, "y_pred": tox.astype(np.float64)})
        return df, "classification"
    raw = models["single"].predict(xgb.DMatrix(X))
    if task == "multiclass":
